...
\vspace{0.5cm}
\textbf{OpenCV}\\
È una libreria software open source (discussa più approfonditamente nella successiva sezione \ref{X}).
...
\vspace{0.5cm}
\textbf{Imutils}\\
La procedura d'installazione della libreria \textit{imutils} (anch'essa descritta con maggior dettaglio nella sezione \ref{X}) è classica:
\begin{lstlisting}
pip install imutils
\end{lstlisting}
...
\vspace{0.5cm}
\textbf{Dlib}\\
Un'ulteriore libreria utilizzata nel progetto è \textit{Dlib}; per essere pienamente operativa, essa ha prima bisogno di alcune dipendenze, reperibili tramite i seguenti comandi (lanciati \underline{fuori} dall'ambiente virtuale di python):
\begin{lstlisting}
sudo apt-get install build-essential cmake
sudo apt-get install libgtk-3-dev
sudo apt-get install libboost-all-dev
