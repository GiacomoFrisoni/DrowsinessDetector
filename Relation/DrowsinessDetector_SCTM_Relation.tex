...
\begin{figure}[!htb]
	\begin{subfigure}{.45\textwidth}
		\centering
		\includegraphics[width=.8\linewidth]{eps/home_circuit.eps}
		\caption{Raspberry Pi in configurazione Desktop per fini di analisi o debug.}
	\end{subfigure}
	\hspace{5mm}
	\begin{subfigure}{.55\textwidth}
		\centering
		\includegraphics[width=.8\linewidth]{eps/car_circuit.eps}
...
\begin{itemize}
	\item \textbf{File video:}\\\texttt{<category\_number>\_<driver>.mp4}
	\item \textbf{Manual drowsiness labels:}\\\texttt{result\_<category\_number>\_<driver>.txt}
	\item \textbf{Detected drowsiness labels:}\\ \texttt{result\_<category\_number>\_<driver>\_<ear\_threshold>\_det.txt}
	\item \textbf{Video confusion matrix:}\\\texttt{result\_<category\_number>\_<driver>\_<ear\_threshold>\_cm.txt}
	\item \textbf{Category confusion matrix:}\\\texttt{<ear\_threshold>\_cm\_tot.txt}
\end{itemize}
...
\begin{figure}[p]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Comparazione tra i valori di EAR in media dei due conducenti. Il soggetto MP in pose con occhi aperti possiede un range di variazione dell'EAR più elevato del soggetto GF. Gli stessi risultati si riflettono nei valori massimi a occhi chiusi.}
	\label{fig:ear_comparing}
\end{figure}
...
\begin{figure}[p]
	...
	\framebox{\begin{tikzpicture}
		...
		\end{axis}
		\end{tikzpicture}}
	\caption{Curva ROC riportante il rapporto tra TPR e FPR al variare dell'$EAR_{th}$.}
	\label{fig:roc}
\end{figure}
...
È interessante osservare come molti ricercatori abbiano condotto i loro esprimenti in ambienti simulati ottenendo alte percentuali di successo, ma come queste siano poi calate drasticamente al passaggio in un ambiente reale. Come già descritto nella sezione \ref{sec:work_planning}, le accuratezza ottenute nel progetto in esame si rivelano pertanto positive.\\
Un limite delle misure comportamentali adottate all'interno di questo progetto è stato individuto da Golz. Il suo team di ricerca ha valutato differenti prodotti commerciali per la rilevazione di fenomeni di sonnolenza ed è giunto a osservare come non sia sufficiente basarsi su una sola tipologia di misura per giudicare lo stato del veicolo e dell'utente alla guida di esso. Di conseguenza, un'ulteriore miglioria che potrebbe essere introdotta nel progetto sviluppato si riferisce alla possibilità di integrare ulteriori parametri per il riconoscimento della sonnolenza stessa (quali angolazione del volto, frequenza del battito cardiaco e superamento della segnaletica orizzontale).\\
...
\includepdf[pages=1,scale=.95]{pdf/drowsiness_detector_gantt.pdf}
...
\thispagestyle{empty}
{
	...
	\captionof{figure}{Diagramma di Gantt.}
	\label{fig:gantt}
	\par
}
