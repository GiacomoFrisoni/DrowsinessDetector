...
Un'ulteriore libreria utilizzata nel progetto è Dlib; per essere pienamente operativa, essa ha prima bisogno di alcune dipendenze, reperibili tramite i seguenti comandi (lanciati \textbf{fuori} dall'ambiente virtuale di python):
\begin{lstlisting}
...
Successivamente, grazie all'utilizzo di \quotes{PIP} si riescono a reperire tutte le altre dipendenze e la libreria Dlib. Per eseguire questi comandi è necessario trovarsi nell'ambiente virtuale di Python precedentemente configurato.
\begin{lstlisting}
pip install numpy
pip install scipy
...
Il progetto presenta numerose e complesse elaborazioni su immagini; avendo a disposizione un buon margine di memoria sulla MicroSD è consigliabile incrementare virtualmente la quantità della RAM disponibile, modificando opportunamente l'attuale configurazione di sistema. Uno dei modi per riservare una parte dello spazio presente sulla schedina è quello di incrementare la dimensione dello \quotes{swap file}. Questo può essere fatto lanciando come amministore il comando:

\begin{lstlisting}
sudo nano /etc/dphys-swapfile
\end{lstlisting}
e sostituendo alla stringa \quotes{CONF\_SWAPSIZE} il valore che si desidera assegnare al file di swap (espresso in megabyte). Nel progetto in questione si è scelto di dedicare un ulteriore gigabyte di spazio:
\begin{lstlisting}
CONF_SWAPSIZE=1024
\end{lstlisting}

Un ulteriore cambiamento alle impostazioni del sistema, utile ad incrementare le performance e ricavare qualche megabyte di memoria, è di decrementare la RAM riservata ai calcoli della GPU, in quanto le elaborazioni effettuate si svolgeranno soltanto sulla CPU. Si può configurare il precedente parametro attraverso la configurazione del Raspberry, accessibile dal comando
\begin{lstlisting}
raspi-config
\end{lstlisting}
sotto la categoria \quotes{Advanced Options => Memory Split}. La dimensione impostata per questo progetto è stata di 16MB: quantità di memoria più che sufficiente per riuscire ad avviare il Raspberry anche in modalità desktop grafico.

Per rendere operative le soprastanti modifiche, è necessario riavviare il Raspberry.
