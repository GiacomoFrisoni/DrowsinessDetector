...
Dopo aver letto i parametri di input, si procede con la configurazione dei pin di GPIO per i tre LED (output) e per il buzzer (input). Tale operazione, data la sua semplicità, non è riportata all'interno della relazione. A seguito del setup dei pin stessi, si effettua il caricamento della conoscenza all'interno del tipo \texttt{CascadeClassifier}.\\
...
\begin{python}
	...
	return ear
\end{python}
Citare disegno sul frame di FPS, EAR e ALERT tramite OpenCV.
